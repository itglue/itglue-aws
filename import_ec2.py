...
def import_ec2_instances(organization, ignore_locations=False):
    ...
    active_status = Resource.first_or_create('configuration_statuses', name='Active')
    inactive_status = Resource.first_or_create('configuration_statuses', name='Inactive')
    ec2_type = Resource.first_or_create('configuration_types', name='EC2')
    # locations = Resource.get('locations', parent=organization) if not ignore_locations else []
    ...
    for instance in instances:
        ...
        if not ignore_locations:
            ...
            if locations_dict.get(location_name):
                ...
                location = Resource.first_or_create('locations', organization_id=organization.id, **location_attributes)
...
def update_or_create_configuration(instance, location, organization, conf_type, active_status, inactive_status):
    ...
    if serial_number:
        configuration = Resource.find_by('configurations', organization_id=organization.id, serial_number=serial_number)

    configuration = configuration or Resource('configurations', organization_id=organization.id)
...
def update_or_create_config_interface(interface, configuration, primary=False):
    ...
    config_interface = Resource.first_or_initialize(
