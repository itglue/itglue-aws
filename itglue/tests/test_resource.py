...
class TestData(object):
    ...
    resource_class = itglue.Resource
    data_dict = {
        'id': resource_id,
        'type': resource_type,
    ...
    resource_from_data = itglue.Resource(
        resource_type,
        id=resource_id,
...
class TestResource(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = patch('itglue.itglue.resource.connection', self.mock_connection)
        patcher.start()
        self.addCleanup(patcher.stop)
        self.resource_class = itglue.Resource
        self.parent_resource = itglue.Resource(TestData.parent_type, name=TestData.parent_name)
        self.resource = itglue.Resource(
            TestData.resource_type,
    ...
    def test_save_new(self):
        with patch.object(itglue.resource.connection, 'post', return_value=TestData.data_dict):
    ...
    def test_save_existing(self):
        with patch.object(itglue.resource.connection, 'patch', return_value=TestData.data_dict):
            self.resource.id = TestData.resource_id
    ...
    def test_create_error(self):
        self.resource.id = TestData.resource_id
        self.assertRaises(itglue.Resource.ResourceError, self.resource.create)
    ...
    def test_create_with_relationships(self):
        with patch.object(itglue.resource.connection, 'post', return_value=TestData.data_dict):
            interface_1 = itglue.Resource('configuration_interface', name='eni-545f123a', ip_address='1.2.3.4')
            interface_2 = itglue.Resource('configuration_interface', name='eni-545f789a', ip_address='5.6.7.8')
    ...
    def test_update_error(self):
        self.assertRaises(itglue.Resource.ResourceError, self.resource.update)
    ...
    def test_update_with_parent_error(self):
        ...
        self.resource.id = TestData.resource_id
        with self.assertRaises(itglue.Resource.ResourceError):
    ...
    def test_get(self):
        with patch.object(itglue.resource.connection, 'get', return_value=TestData.data_list):
            expected_path = '/{}'.format(TestData.resource_type)
            self.assertEqual(self.resource_class.get(TestData.resource_type), [TestData.resource_from_data])
    ...
    def test_get_with_parent(self):
        with patch.object(itglue.resource.connection, 'get', return_value=TestData.data_list):
            ...
            resources = self.resource_class.get(TestData.resource_type, parent=self.parent_resource)
    ...
    def test_filter(self):
        with patch.object(itglue.resource.connection, 'get', return_value=TestData.data_list):
            ...
            resources = self.resource_class.filter(TestData.resource_type, name=TestData.resource_name)
            self.assertEqual(resources, [TestData.resource_from_data])
    ...
    def test_find(self):
        with patch.object(itglue.resource.connection, 'get', return_value=TestData.data_dict):
            expected_path = '/{}/{}'.format(TestData.resource_type, TestData.resource_id)
            resource = self.resource_class.find(TestData.resource_type, TestData.resource_id)
            self.assertEqual(resource, TestData.resource_from_data)
            self.mock_connection.get.assert_called_once_with(expected_path)
    ...
    def test_find_with_parent(self):
        with patch.object(itglue.resource.connection, 'get', return_value=TestData.data_dict):
            ...
            resource = self.resource_class.find(TestData.resource_type, TestData.resource_id, parent=self.parent_resource)
            self.assertEqual(resource, TestData.resource_from_data)
            self.mock_connection.get.assert_called_once_with(expected_path)
    ...
    def test_first_or_create_with_match(self):
        with patch.object(itglue.resource.connection, 'get', return_value=TestData.data_list):
            expected_path = '/{}'.format(TestData.resource_type)
            expected_params = { 'filter': { 'name': TestData.resource_name } }
            resource = self.resource_class.first_or_create(TestData.resource_type, name=TestData.resource_name)
            self.assertEqual(resource, TestData.resource_from_data)
            self.mock_connection.get.assert_called_once_with(expected_path, params=expected_params)
            self.mock_connection.post.assert_not_called()

    def test_first_or_create_without_match(self):
        with patch.object(itglue.resource.connection, 'get', return_value=[]), \
                patch.object(itglue.resource.connection, 'post', return_value=TestData.data_dict):
            expected_path = '/{}'.format(TestData.resource_type)
            expected_params = { 'filter': { 'name': TestData.resource_name } }
            expected_payload = {
                'type': TestData.resource_type,
    ...
    def test_first_or_initialize_with_match(self):
        with patch.object(itglue.resource.connection, 'get', return_value=TestData.data_list):
            expected_path = '/{}'.format(TestData.resource_type)
            expected_params = { 'filter': { 'name': TestData.resource_name } }
            resource = self.resource_class.first_or_initialize(TestData.resource_type, name=TestData.resource_name)
            self.assertEqual(resource, TestData.resource_from_data)
            self.mock_connection.get.assert_called_once_with(expected_path, params=expected_params)
            self.mock_connection.post.assert_not_called()
    ...
    def test_first_or_initialize_without_match(self):
        with patch.object(itglue.resource.connection, 'get', return_value=[]):
            ...
            expected_params = { 'filter': { 'name': TestData.resource_name } }
            resource = self.resource_class.first_or_initialize(TestData.resource_type, name=TestData.resource_name)
            self.assertEqual(resource, itglue.Resource(TestData.resource_type, name=TestData.resource_name))
            self.mock_connection.get.assert_called_once_with(expected_path, params=expected_params)
            self.mock_connection.post.assert_not_called()
    ...
    def test_find_by(self):
        with patch.object(itglue.resource.connection, 'get', return_value=TestData.data_list):
            expected_path = '/{}'.format(TestData.resource_type)
            expected_params = { 'filter': { 'name': TestData.resource_name } }
            resource = self.resource_class.find_by(TestData.resource_type, name=TestData.resource_name)
            self.assertEqual(resource, TestData.resource_from_data)
            self.mock_connection.get.assert_called_once_with(expected_path, params=expected_params)
    ...
    def test_find_by_no_attributes_error(self):
        with self.assertRaises(itglue.Resource.ResourceError):
            self.resource_class.find_by(TestData.resource_type)

    def test_find_by_all_empty_error(self):
        with self.assertRaises(itglue.Resource.ResourceError):
            self.resource_class.find_by(TestData.resource_type, name=None, notes='')
